...
ipcMain.on('open-pdf-window', (event, pdfDocument) => {
    if (pdfDocument) {
        ...
        pdfWindow.loadURL('file://' + __dirname + '/app/components/pdfjs/web/viewer.html?file=' + pdfDocument);
        ...
        event.sender.send('open-pdf-window-callback');
    } else {
        event.sender.send('open-pdf-window-callback', 'no pdf file');
    }
})
